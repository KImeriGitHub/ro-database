"""Estimate API calls and time for INDEX_DATA historical fetch.

One API call per index symbol (~400+ symbols). Queries INDEX_CATALOG to
get the full symbol list, then samples real calls to measure round-trip time.

Usage:
    python tests/call_speedtests/estimate_indices_calls.py
    python tests/call_speedtests/estimate_indices_calls.py --max-calls 10
"""
...
def _fetch_index_symbols(api_key: str) -> list[str]:
    """Query INDEX_CATALOG, return list of index symbols."""
    print("Querying INDEX_CATALOG...")
    resp = requests.get(
        f"{AV_BASE}/query?function=INDEX_CATALOG&apikey={api_key}", timeout=60,
    )
...
def main(
    ...
    symbols = _fetch_index_symbols(api_key)
    total_symbols = len(symbols)

    print(f"Indices: {total_symbols} symbols")
    ...
    print()
    print(
        "Note: INDEX_DATA was introduced in April 2026 and may not work"
        " with older premium API keys."
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Estimate INDEX_DATA API call count and time"
    )
    parser.add_argument(
