"""Estimate API calls and time for DIGITAL_CURRENCY_DAILY historical fetch.

One API call per cryptocurrency symbol (~600 USD pairs). Queries the AV
cryptocurrency list to get all symbols, then samples real calls to measure
round-trip time.

Usage:
    python tests/call_speedtests/estimate_cryptocurrencies_calls.py
    python tests/call_speedtests/estimate_cryptocurrencies_calls.py --max-calls 10
"""
...
def _fetch_crypto_symbols() -> list[str]:
    """Query cryptocurrency_list, return USD-market symbols."""
    print("Querying cryptocurrency_list...")
    csv_text = requests.get(f"{AV_BASE}/cryptocurrency_list/", timeout=60).text
    raw = pl.read_csv(io.StringIO(csv_text))
    usd_only = raw.filter(pl.col("to_currency") == "USD")
    return usd_only["from_currency"].to_list()
...
def main(
    ...
    symbols = _fetch_crypto_symbols()
    total_symbols = len(symbols)

    print(f"Cryptocurrencies: {total_symbols} symbols (USD pairs)")
    print(f"Total API calls needed: {total_symbols} (1 per symbol)")
    print()

    sample_size = max_calls if max_calls is not None else 100
    ...
    print()
    print(
        "Note: many crypto symbols may be dead/empty. The ingestion report"
        " captures which ones fail. Actual time may be lower than estimated."
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Estimate DIGITAL_CURRENCY_DAILY API call count and time"
    )
    parser.add_argument(
