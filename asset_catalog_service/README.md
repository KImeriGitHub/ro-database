...
- **Re-issued tickers (active wins):** AV's `LISTING_STATUS` can return the same `symbol` string twice when a ticker was reused: once in `state=delisted` for the original company (with its old `ipoDate` and a `delistingDate`) and once in `state=active` for the new company that now trades under it (with a newer `ipoDate` and null `delistingDate`). `_fetch_av_listings` deduplicates by symbol with active winning, so the catalog reflects the currently-trading entity and the daily yield-aware pipeline does not skip a live ticker. The historical record of the prior issuer is not kept here; price history for the old company still lives under `historical/` keyed by date. Note: `symbol` is treated as a primary key throughout the catalog, so representing both issuers as separate rows would require a richer key (e.g. `symbol + ipoDate`) and is out of scope.
