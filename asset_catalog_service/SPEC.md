...
- **Stock/ETF collision logging:** AV occasionally classifies the same symbol as both a stock and an ETF in one run. `update_stocks_etfs` warns only when the symbol is `Active` on both sides; a collision with a `Delisted` side is just a reissued ticker and is not logged. The warning is informational, no side is dropped.
