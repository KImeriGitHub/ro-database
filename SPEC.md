...
### GCS transfer reliability

The historical push moves 16k+ small parquet files in one run, so the transfer helpers in [maintainance_scripts/gcs_client.py](maintainance_scripts/gcs_client.py) are built to survive a slow or flaky uplink without losing progress.

- **Connection pool sized to the worker count.** `storage.Client` mounts urllib3's default HTTP adapter, capped at `pool_maxsize=10`. Running more `--workers` than that overflows the pool, so every extra transfer discards and re-opens a TLS socket on each request -- the "Connection pool is full, discarding connection" warnings -- and that churn destabilises sockets into write timeouts. `_ensure_pool_size(workers)` grows the shared client's HTTPS pool to match the worker count (floor of 10, never shrinks) before either tree helper starts its thread pool, so each worker keeps a warm connection. It is called from the main thread before any worker starts, so no locking is needed, and it tolerates a client without a mountable `_http` session (stubs in tests).
- **Resumable uploads.** `upload_tree` defaults to `skip_if_same_md5=True`: it fetches existing blob metadata in one `list_blobs` pass and skips any file already present with a matching MD5, mirroring `download_tree`'s skip logic. An interrupted push (network blip, write timeout, manual kill) resumes on re-run by sending only what is missing or changed, instead of re-pushing every file. A remote blob without an `md5_hash` (composite objects expose none) is always re-uploaded since freshness cannot be verified. The `--force` size-mismatch guard in [scheduled_scripts/push_historical_to_gcs.py](scheduled_scripts/push_historical_to_gcs.py) is unaffected: completed files match on size, so a resumed run passes the guard and the MD5 skip handles the rest. `upload_tree` returns only the blobs it actually uploaded; no caller depends on the full list.
- **Per-transfer timeout and retry budget.** Each `upload_from_filename` / `download_to_filename` runs with a 300s per-request timeout and a 600s total retry deadline (`_TRANSFER_TIMEOUT` / `_TRANSFER_RETRY`), replacing the library defaults (60s request, 120s retry deadline) that aborted a single slow large-file chunk and took the whole tree down with it.
- **Recommendation** Do NOT use more than 8 workers.
