"""Download historical commodity data from Alpha Vantage."""
...
# Symbols where daily interval is available -- fetch daily.
_DAILY_SYMBOLS = {"WTI", "BRENT", "NATURAL_GAS"}

# Symbols where finest granularity is monthly.
_MONTHLY_SYMBOLS = {
    "COPPER", "ALUMINUM", "WHEAT", "CORN",
    "COTTON", "SUGAR", "COFFEE", "ALL_COMMODITIES",
}

# Gold/Silver use a different AV function.
_GOLD_SILVER_MAP = {
    "XAU": "GOLD",
    "XAG": "SILVER",
}
...
async def _fetch_standard(
    ...
    """Fetch a standard commodity (WTI, BRENT, ... ALL_COMMODITIES)."""
    url = (
...
async def _fetch_gold_silver(
    ...
    """Fetch gold (XAU) or silver (XAG) via GOLD_SILVER_HISTORY."""
    url = (
...
async def fetch_commodities(
    ...
    """Download historical data for all commodities in the catalog."""
