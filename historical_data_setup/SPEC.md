...
Every stock/ETF endpoint queries the full catalog -- both `status == "Active"` and `status == "Delisted"` symbols are fetched. Historical setup is the survivorship-bias-free anchor, so delisted coverage matters; the daily service is the one that restricts to active. The only per-symbol exception is `sentiment`, whose per-symbol parquet split is active-only (the global `ALL_MESSAGES.parquet` still carries every catalog symbol). `forex`, `indices`, `cryptocurrencies`, `commodities`, and `economic` ignore the `status` column entirely (their catalogs only carry currently-listed instruments).
...
Per symbol, fetches insider transaction history for every stock in the catalog (active and delisted). The API response contains a flat `"data"` list. The `"transaction_date"` field is renamed to `transactionDate`; the `"ticker"` field is dropped (redundant with the file name). One file per symbol.
...
| insider | 0.70s | 10 | 8,649 | ~1.9 h | All stocks (active + delisted); ~1,150 rows/call |
...
- **Fundamental endpoints** (income_statement, balance_sheet, cash_flow, earnings, earnings_estimates): `_build_fundamental_df` in `_common.py` replaces all null sentinels (`None`, `"None"`, `""`, `"."`) with Python `None` before constructing the DataFrame with `infer_schema_length=None` (full scan; columns whose first value is null still resolve to Utf8 because every value is a Python string post-cleanup). Polars null values in a Utf8 column cast to Float32 as null -- no error raised. Columns that fail to cast to Float32 after sentinel cleanup are force-cast with `strict=False` (remaining non-castable values become null) and a `cast_failure` is recorded. Columns are **never** left as String when Float32 is expected.
- **Insider endpoint**: Same null sentinel replacement before DataFrame construction with `infer_schema_length=None`. Known string columns (`executive`, `executive_title`, `security_type`, `acquisition_or_disposal`) are kept as String; all others must be Float32. Non-castable values in Float32 columns are force-cast to null with a `cast_failure` recorded.
